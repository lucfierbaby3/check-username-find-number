...
async def check_phone(phone_to_test: str, username_to_check: str):
    ...
    try:
        ...
        contact = types.InputPhoneContact(
            client_id=0, phone=phone_to_test, first_name="Temp", last_name="Temp"
        )
        try:
        ...
        if found_username:
            ...
            print(
                f"شماره متعلق به کاربری است اما یوزرنیم ندارد. نام نمایشی: {found_name}"
            )
            return False
        ...
        try:
            await client(
                functions.contacts.DeleteContactsRequest(
                    id=[types.InputUser(user_id=0, access_hash=0)]
                )
            )
            ...
            try:
                ...
                for c in contacts:
                    if getattr(c, "phone", None) and c.phone.replace(
                        "+", ""
                    ) == phone_to_test.replace("+", ""):
